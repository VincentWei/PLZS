...
- 课程共包含 20 场直播，每场两个课时（每课时 40 分钟，中间休息 10 分钟）。一般每周五 20:30 开讲。
...
- `python3`：Python 3 的解释器（interpreter）；可以交互模式执行 Python 程序或者以解释器模式执行指定的 Python 程序。
...
- 插入（insert）模式：用于键入文本，按 ESC 键退出插入模式进入常规模式。
...
- `n`　向下重复上个搜索。
- `N`　向上重复上个搜索。
- `:%s/WORD1/WORD2/gc`　全局搜索 WORD1 并使用 WORD2 替换。`gc` 是可选后缀　`g` 表示替换行内所有搜索到的 WORD1，`c` 表示需要用户确认是否替换。
...
空白字符
   - 空白字符（空格或者制表符）用于分隔语句中的关键词、名称和表达式，是必须的。
   - 空白字符还可以用来分隔一个表达式中的不同组成部分。在不引起歧义的情况下可不使用空格，空格的多少也无关紧要。
   - 必要时使用空格，可有效提升代码的阅读体验。
...
1. 整理自己所学的几何公式，将其写成对应的 Python 函数，程序调用这些函数，提示用户输入需要的参数值，求值并打印对应的结果。要求使用 VIM 编辑源文件。
