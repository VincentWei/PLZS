...
- 观看直播时可放大画面；不允许在观看直播时操作电脑，以免打断思路。
...
1. 变量名必须以字母或者下划线（underline，`_`）打头，然后可使用字母、数字或下划线，但不能包含除下划线之外的其他符号。
...
- Python 使用 `def` 语句定义一个函数，一个函数包括函数名（其命名规则和变量名类似）及其形参（parameter），形参可以没有，也可以有多个，用逗号（comma，`,`）分隔。
...
- 可执行（executable）程序。
- 脚本（script）程序。
- 在脚本开头使用 `#!/bin/python3` 这样的方式指定脚本程序的解释器。
