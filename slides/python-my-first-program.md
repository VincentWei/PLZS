...
看演示视频[1-1：初识 Ubuntu Linux 桌面环境](#)：
...
1. Linux 或类 UNIX 系统中文件的路径（path）使用 `/` 符号分隔目录，而 Windows 使用盘符和 `\`。
1. 大部分情况下可用 `Ctrl+C`（`^C`） 终止一个正在运行的命令；如果是全屏运行的程序，大部分情况下可以按 `q` 键退出。
1. 大部分情况下可用 `--help` 选项来显示一个命令的简易使用说明。
1. 命令使用说明文档中的一些约定或习惯写法：
   - Linux 命令是区分大小写的，通常使用小写字母。要注意字符的全角（fullwidth）和半角（halfwidth）的区别。
