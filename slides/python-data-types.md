...
4) 针对字符串的运算符亦可用于列表和元组：拼接（`+`）、重复（`*`）、索引值引用（`[]`）、切片（`[:]`）、成员检测（`in`）、非成员检测（`not in`）。
5) 使用 `l += other` 可将 `other` 列表的内容追加到 `l` 列表之后。

	
[3-3：Python 数据类型：列表和元组](#)（续）
...
9) 可使用 `del list[INDEX]` 语句移除一个列表成员。
...
3) 使用 `dict[key]` 这样的语法引用一个键值对的值；字典中的键值对是无序的，无法使用索引值引用。
