...
8) 针对列表的内置方法：
   - `s.append(x)`：在 `s` 的尾部追加新单元 `x`。
   - `s.clear()`：清除所有成员，变成空列表。
   - `s.copy()`：复制列表。
   - `s.count(x)`：返回 `x` 在列表中出现的次数。
   - `s.extend(t)`：用 `t` 的内容扩展 `s`。
   - `s.index(x)`：搜索并返回和指定数据 `x` 匹配的（matched）第一个成员之索引值。
   - `s.insert(i, x)`：插入数据 `x` 到指定的位置 `i`。
   - `s.pop()`：提取最后一项，并将其从 `s` 中移除。
   - `s.pop(i)`：提取在 `i` 位置上的项，并将其从 `s` 中移除。
   - `s.remove(x)`：移除和指定数据 `x` 匹配的第一个项。
   - `s.reverse()`：就地将列表成员逆序。
   - `s.sort()`：对列表中的成员排序。

	
9) 针对元组的内置方法：
   - `s.count(x)`：返回 `x` 在元组中出现的次数。
...
7) `del dict[KEY]` 语句移除一个键值对。
8) `key in d`：键检测；如果 `d` 中存在键 `key` 则结果为 `True`，否则为 `False`。
9) `key not in d`：非键检测；如果 `d` 中存在键 `key` 则结果为 `False`，否则为 `True`。
