...
1. 一个字典由一组属性构成，每个属性由一个名称（通常为字符串）和对应的值构成，属性又被称为“键值对（key-value pair）”。
1. 一个字典中的属性名（键）不可重复，我们可通过属性名（键）来引用对应的值。
1. 在其他编程语言中，可能使用对象（object）这一术语或者关联数组（associated array）这一术语。

	
### 讲解视频[3-4：Python 数据类型：字典](#)

1. Python 中的字典使用成对出现的大括号（braces，`{}`）定义，各个属性之间使用逗号（comma，`,`）分隔，属性名和属性值之间使用冒号（colon，`:`）分隔。
1. 在 Python 中使用 `dict['<key-name>']` 这样的语法引用一个属性的值。

	
要点总结

1. 字典中的属性名可以是除字符串之外的其他数据类型，如整数。
1. 字典中的属性值可以是任意数据类型。
1. 字典中的属性是无序的，无法使用索引值引用。
1. 在字典数据上使用 `for` 语句默认遍历属性值。
