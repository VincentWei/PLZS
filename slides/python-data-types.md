...
- 列表中的一个数据项（data item）也会被称为一个单元（unit）或一个成员（member）。
...
3) 针对列表和元祖的操作符：拼接（`+`）、重复（`*`）、索引值引用（`[]`）、切片（`[:]`）、成员检测（`in`）、非成员检测（`not in`）。
4) 列表中的数据项可以是任意数据类型，也就是列表中可以包含另一个列表。
5) 元组中的单元不可改变，但若其中的单元是一个列表，则这个列表中的数据项可以改变。
6) `+` 和 `*` 可以作用于列表和元组，分别用于拼接（concatenate）两个列表/元组，或者扩大列表/元组 N 倍；这两种操作均构成一个新的列表/元组。
...
8) 针对列表的内置方法：
   ...
   - `.clear()`：清除所有成员，变成空列表。
   - `.copy()`：复制列表。
   - `.count()`：计算列表中和指定数据匹配（matched）的个数。
   - `.extend()`：在尾部
   - `.index()`：搜索并返回和指定数据匹配的（matched）第一个成员之索引值。
   - `.insert()`：插入数据到指定的位置。
   - `.pop()`：弹出最后一个成员。
   - `.remove()`：移除和指定数据匹配的第一个成员。
   - `.reverse()`：反转列表成员的顺序。
   - `.sort()`：对列表中的成员排序。
9) 针对元组的内置方法：
   - `.count()`：计算元组中和指定数据匹配的个数。
   - `.index()`：搜索并返回和指定数据匹配的第一个成员之索引值。
...
1. 计算机程序中的字典（dictionary）通常用来表达具有多重属性（property）的复杂对象，比如一名学生的学号、姓名、性别、生日、身高、体重等。
1. 一个字典由一组键值对（key-value pair）构成，每个键值对由一个键（通常为字符串）和对应的值构成。
1. 一个字典中的键不可重复，我们可通过键来引用对应的值。
1. 在其他编程语言中，可能使用对象（object）这一术语或者关联数组（associated array）这一术语；键值对被称为“属性（property）”。

	
### 讲解视频[3-4：Python 数据类型：字典](#)

1) Python 中的字典使用成对出现的大括号（braces，`{}`）定义，各个键值对之间使用逗号（comma，`,`）分隔，键和值之间使用冒号（colon，`:`）分隔。
2) 在 Python 中使用 `dict[KEY]` 这样的语法引用一个键值对的值。
3) 字典中的键可以是除字符串之外的其他数据类型，如整数。
4) 字典中的值可以是任意数据类型。
5) 字典中的键值对是无序的，无法使用索引值引用。
6) 在字典数据上使用 `for` 语句默认遍历值。
...
8) 针对字典的内置方法：
   - `.clear()`：清除字典中的所有键值对。
   - `.copy()`：复制字典。
   - `.fromkeys()`：使用指定的键元组以及可选的值构建一个字典。
   - `.get()`：获取指定键的值；若不存在返回 `None`。
   - `.items()`：返回构成的键值对列表，其中的每个单元是一个对应键、值构成的元组。
   - `.keys()`：返回字典的键列表。
   - `.pop(obj,)`：弹出指定属性名对应的属性，返回属性值。
   - `.popitem()`：删除最后插入字典的键值对，返回属性值。
...
1. `x in c`：成员检测；如果 `c` 中的某项等于 `x` 则结果为 `True`，否则为 `False`。
1. `x not in c`：非成员检测；如果 `c` 中的某项等于 `x` 则结果为 `False`，否则为 `True`。
...
1. 针对列表的内置方法同样适用于其他可变序列：字节组
1. 针对元组的内置方法同样适用于其他不可变序列：字符串、字节们
