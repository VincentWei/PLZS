...
1. 立即数（immediate value）的概念。
1. Python 中的整数可使用二进制（binary，`0b`）、八进制（octonary，`0o`）、十六进制（hexadecimal，`0x`）写法。
1. Python 的 `print()` 函数显示浮点数时，始终会包含小数点。
1. 所有带有小数点的立即数被视作浮点数。
1. 可以用类似科学计数法的方式定义一个立即数：`1E10` 或者 `33E2`，此时该立即数始终是浮点数。
1. 大部分算术运算符的结果数据类型取决于操作数（operand）的类型：操作数中有浮点数时，结果一定是浮点数。
1. 针对数值的常用内置函数：
   - `int()`、`float()`、`bool()`：转换为对应的数值类型。这几个函数本质上是 `int`、`float` 和 `bool` 类型的构造函数（constructor）。
   - `abs(x)`：取绝对值；`max(x1, x2, ...)`：取最大值；`min(x1, x2, ...)`：取最小值；`round(x[, n])`：四舍五入；`pow(x, y)`等价于 `x ** y`。
1. 针对浮点数运算，可导入 Python 的内置模块 `math`：`import math`，其中包含有很多常用的数学函数，比如三角函数；使用 `dir()` 函数可展示模块提供的接口（interface）。
   - `math.sqrt()`：求平方根；`math.isclose(x, y)`：判断两个浮点数是否接近（考虑表达误差）。
1. `math.pi` 是 `math` 模块定义的常亮，即圆周率；类似的还有 `math.e`（自然常数）。
1. 复数（complex）是高等数学引入的一种数值类型，在 Python 中使用 `complex()` 构造复数；进行复数运算时，可导入内置的 `cmath` 模块。
...
- Python 的字符串使用成对出现的单引号（single-quotes，`''`）或者双引号（double-quote，`""`）定义。
...
1. 可调用（callable）类型指函数或者方法等：
  - 使用 `def` 语句定义的函数便是一个可调用类型。
  - 内置的函数或方法。
  - 用户自定义的类、构造函数及其方法。
