...
- 初始化一个大小为 `n` 的数组 `queue`，其中 `n` 是环形队列可以容纳的最大元素数。
- 将 `front` 和 `rear` 两个变量初始化为 `-1`。
- 入队：要将元素 `x` 加入队列，执行以下操作：
  1. 首先判断队列是否已满（思考如何判断）。如果队列已满，可返回错误消息，或者丢弃最早加入的元素（即 `front` 指向的元素）。
  1. 如果队列未满，则 `rear` 增加 `1`；如果 `rear` 等于 `n`，则将 `rear` 设置为 `0`。
  1. 将 `queue[rear]` 设置为 `x`。
  1. 如果 `front` 为 `-1`，则将 `front` 设置为 `0`。
- 出队：要从队列中取出元素，执行以下操作：
   1. 通过检查 `front` 是否为 `-1` 来判断队列是否为空；如果是，则返回一条错误消息，指示队列为空。
   1. 将 `x` 设置为 `queue[front]`。
   1. 如果 `front` 等于 `rear`，则将 `front` 和 `rear` 均设置为 `-1`（表明队列已空）；否则将 `front` 增加 `1`，如果 `front` 等于 `n`，则将 `front` 设置为 `0`。
   1. 返回 `x`。
...
1) 针对单向链表实现 `unique()` 函数，该函数可移除单向链表中所有重复的值，然后编写用来展示该功能的程序：用户每行输入一个浮点数，直到输入 `END` 为止，最后给出经 `unique()` 函数处理后的链表内容。运行效果如下：
...
3) 使用 STL 的 `std::deque` 类模板解析前缀表达式（仅支持加减乘除四种运算）并求值。运行效果如下：

```console
$ ./evaluate-prefix-expression-deque
<- / + 6 * 2 3 4 5>     # 前缀表达式，使用单个空格字符分隔运算符及操作数；下同
-2
```
...
<500 100 200 + ->       # 后缀表达式，使用单个空格字符分隔运算符及操作数；下同
200
```
