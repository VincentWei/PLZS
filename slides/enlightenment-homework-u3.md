...
算始 生成质数序列 (n)
    ...
    当始 (k <= n)
        ...
        # 若 flag 未被修改为 假，则表明没有已知质数可以整除 k，
        # 也就是说 k 是质数
        若始 (flag == 真)
            primes := primes + [k]
        若终
...
- 证明：
   1) 根据合数的定义，给定的任意合数 `$ n $` 可以分解为两个整数之乘积，且这两个整数均不为 `$ 1 $` 和 `$ n $`。也就是说有：

`$$
    n = a \times b,\ (1 < a < n,\ 1 < b < n)
$$`

   2) `$ a $` 和 `$ b $` 有两种情况：第一，全是质数；第二，有一个是质数另一个是合数；第三，全是合数。若是前两种情况，则得证。
   3) 对第三种情况，取其中一个合数，设其为 `n_1`，然后继续分解 `$ n_1 $` 并判断其两个因子。
   4) 重复以上过程 `$ m $` 次，并假设每次都得到两个合数的因子（步骤 2 中的第三种情况），则必然有 `$ 1 < n_{m + 1} < n_m < \cdots < n_1 < n $`。
