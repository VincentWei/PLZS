...
- 原理
   ...
   1. 有多个运算符参与的混合运算，按顺序、优先级或者括号递归处理，然后将这些二叉树铰接在一起；即将某个二叉树根铰接到另一个二叉树上，成为其子节点。
   1. 通过后序遍历求出所有内部节点的值，根节点的值即为最终求值结果。
- 算法描述
...
- 要求支持如下功能：
  ...
  1. 将中缀表达式解析为求值二叉树进行求值，按求值二叉树输出其中缀表达式及其求值结果。
  1. 若表达式有误，输出 `INVALID`。
- 运行效果如下：
...
Postfix expression: 50.6 300.5 +
    = 351.1
By using evaluation binary tree: (300.5 + 50.6)
    = 351.1
