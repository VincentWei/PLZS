...
若 `$ gcd(a, m) = 1 $`，则 `$ a^{\phi(m)} \equiv 1 \quad (\mod m) $`。其中，欧拉函数 `$ \phi(n) $` 表示的是小于 `$ n $` 且和 `$ n $` 互质的整数之个数。
...
若 `$ p $` 为素数，且 `$ gcd(a,p)=1 $`，则 `$ a^{p-1} \equiv 1 \quad (\mod p) $`。  
或者，若 `$ p $` 为素数，对于任意整数 `$ a $`，有 `$ a^p \equiv a \quad (\mod p) $`。
...
对于素数 `$ p $` 有 `$ (p-1)! + 1 \equiv 0 \quad (\mod p) $`。
