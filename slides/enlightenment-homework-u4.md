...
- 证明（反证法）：
   ...
   1. 也就是 `$ a^2 = 3^{2n}k^{2m} = (3^2)^nk^{2m} $`，可推出 `$ a = 3^n k^m $`；其中 `$ n $` 正整数，`$ k $` 是 `$ a^2 $` 的其他质因子之乘积，`$ k \ge 1,\ m \ge 0 $`。
   1. 根据 `$ a^2 = 3^{2n}k^{2m} = 3b^2 $`，可得 `$ b^2 = 3^{2n-1}k^{2m} $`。
   1. 因为 `$ n > 0 $`，所以 `$ b^2 $` 也是一个三倍数。同理可得 `$ b $` 一定也是三倍数。
