...
- 可执行考鼎码（Executable Coding Code）是一种主要用于少儿信息学启蒙的编程语言；在不引起混淆的情况下可简称考鼎码（Coding Code），简称 `C2`。
- 考鼎码的主要特征为：
   - 解释型脚本编程语言。
   - 支持汉字关键词及命名。
   - 弱类型；主要类型有布尔、整数（系统位宽一致的整数）、浮点数、字节、字符、字符串和序列。
   - 跨平台；优先支持 WASM 平台。
- 中期规划：
   - 任意整数（任意精度整数）
   - 映射
...
- 算法的参数应由解释器提示用户输入并传入，类型为字符串。
- 算法的返回值应视作程序的返回值，按 JSON 格式输出。
...
1. 序列的定义：一组有序的元素构成，元素的数据类型可以是字节、字符、整数、任意整数、浮点数、任意浮点、序列、映射等。
...
- 支持使用如下方式之一定义字节序列字面量：
   1. 使用 `bb` 前缀用二进制定义字节序列字面量（可在中间添加 `.` 作为分隔符）。
   1. 使用 `bx` 前缀用十六进制定义字节序列字面量。
   1. 使用 `b64` 前缀用Base64编码定义字节序列字面量。
...
1. 字符串：内部使用 UTF-8 编码；可视作字节构成的序列，但使用 `\0` 字符作为终止字符，且只能包含合法的 Unicode 字符。
1. 使用双引号（`""`）定义字符串字面量，并使用类似 JSON 字符串的方式使用转义字符和 Unicode 码点。
1. 可通过索引值访问字符串中的每个字节。
...
1. 类似 Python 字典类型，可使用整数、字符串等为键名。
