...
- 考鼎码的主要特征为：
   - 解释型脚本编程语言。
   - 支持汉字关键词及命名。
   - 弱类型；主要类型有整数（系统位宽一致的整数）、浮点数、字节、字符、字符串和序列。
   - 跨平台；优先支持 WASM 平台。
- 中期规划：
   - 任意整数（任意精度整数）
   - 映射
...
1. 字符串：内部使用 UTF-8 编码；可视作字节构成的序列，但使用 `\0` 字符作为终止字符，且必须包含合法的 Unicode 字符。
