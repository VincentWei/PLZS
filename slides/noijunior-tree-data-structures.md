...
- 节点高度（height）：节点的高度可以定义为从根节点到该节点的最长路径的长度；注意根节点高度可以定义为 1 也可以定义为 0。本课件中，若无特别说明，根节点的高度为0，此时节点的深度和高度相同。
- 树的高度（height）：树的高度是所有叶子节点高度的最大值。
...
1. 除最后一层外，所有层都被填满。
1. 高度为 `$ h $` 的完全二叉树，最后一层的节点数量小于 `$ 2^h $`。
1. 高度为 `$ h $` 的完全二叉树节点数量始终小于高度为 `$ h $` 的完美二叉树节点数量。
1. 高度为 `$ h $` 的完全二叉树，其节点数量小于等于 `$ 2^{h+1} - 2 $`。
1. 有 `$ N $` 个节点的完全二叉树，其高度为 `$ \lfloor log_2{N} \rfloor $`。
...
- 映射（mapping）用于建立一个值和另外一个值之间的映射关系；比如身份证号和包括姓名、性别、年龄等在内的单项或多项个人资料，程序可通过身份证号来访问对应的个人资料。
- STL `map`（`<map>`）是一种非线性容器，用于包含多个键值对（key-value pairs）；会对键排序。
- STL `unordered_map`（`<unordered_map>`）和 `map` 的功能类似，但不对键排序。
- 对键排序的效果是，当我们使用迭代器遍历映射中的元素时，将按键的大小顺序给出（默认升序）；不对键排序时，迭代器的迭代顺序是不确定的。
- STL 映射的功能同 Python 中的字典（dict）数据类型类似；在映射中维护的元素亦称属性（property）。
