...
- 4) 编写考鼎伪代码算法，将给定的分数（依次输入分子和分母）化为最简分数形式，输出其整数部分以及最简分数的分子及分母。
...
- 实数范围内的封闭运算：加法、乘法、减法、除法、开方（偶数次方对正数进行）、对数
...
- 2) 编写考鼎伪代码算法，计算两个分数相加的结果并化为最简分数。对分数，程序应分别输入分子和分母。
...
- 2) 将 `$ 0.1 $` 转为二进制。
- 3) 编写考鼎伪代码算法，将一个无限循环小数转为分数。程序应分别输入整数部分、小数中的不循环部分及循环部分，如 `$ 100.356\dot{1}\dot{7} $` 分为三部分输入：`$ 100 $`、`$ 356 $` 和 `$ 17 $`。
...
- 为避免和已有的算法定义方法相混淆，我们规定 `算始/算终` 定义的算法为单个考鼎伪代码程序的唯一入口。
- 也就是说，一个考鼎伪代码程序可以定义多个 `函数`，但只能包含一个 `算法`。
...
- 主播：咱用考鼎伪代码写的算法，稍微做点改动就能执行起来。来看白板。
- 讲解要点：
   1. 考鼎码在线练习场
   1. 函数库的概念。
   1. 考鼎码中如何调用函数库中的函数。
...
- 可执行的[考鼎码（Coding Code，C2）](enlightenment-spec-of-coding-code.html))基于考鼎伪代码（Coding Pseudo Code）定义，保留了大部分的设计，但语法要比伪代码严格。和伪代码的主要区别：
   1. `输入` 被定义为内置函数。
   1. `输出` 被定义为内置函数。
   1. `执行` 被定义为内置函数，该函数执行指定的系统功能；本质上，伪代码中给出的 `拍手`、`说出` 等动作是系统功能的名称，故而需要添加引号。
   1. `终止` 被定义为内置函数。
- 程序中需要用到人名、地名等使用自然语言描述的数据时，通常使用引号包围，表示这些数据和关键词、程序定义的函数、算法等的名称不同。这类数据称作“字符串”，本质上是字符构成的序列。
...
2) 在考鼎码在线练习场中运行自己写过的算法，注意修改伪代码中不规范的写法。
