...
- 3) 编写考鼎码算法，将给定的分数（依次输入分子和分母）化为最简分数形式，分别输出其整数部分以及最简分数的分子及分母。
...
- 要点：
   1. 加减法：对齐小数点做竖式加减法。
   1. 乘法：去掉小数点乘除，再确定小数部分位数（`$ 1.35 \times 12.5678 $`）：

`$$
\begin{align}
    \because &1.35 = 135 \times 10^{-2}  \quad 12.5678 = 125678 \times 10^{-4} \\
...
$$`<!-- .element: class="fragment fade-in" -->
   ...
   1. 除法：同时放大被除数和除数以去掉除数的小数点，然后再用长除法（`$ 1.35 \div 12.5678 $`）：：
