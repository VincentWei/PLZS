...
- <https://courses.fmsoft.cn/iec-playground>
- 和伪代码的区别：
   1. `输入` 被定义为内置函数。
   1. `输出` 被定义为内置函数。
...
```mathematica
算始 数数游戏必胜
    # 实际调用的是 `输入` 函数；因为不需要传入参数，故而省略了括号。
    x := 输入
