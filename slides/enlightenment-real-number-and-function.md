...
- [可执行的考鼎码（Executable Coding Code，EC2）](enlightenment-spec-of-executable-coding-code.html)基于考鼎伪代码（Coding Pseudo Code）定义，保留了大部分的设计，但语法要比伪代码严谨。和伪代码的主要区别：
   ...
   1. `长度()` 函数可用来返回序列的长度。
   1. 数据区分更加严格的类型：整数、浮点数、字节、字符、序列、字符串等。
- 程序中需要用到人名、地名等使用自然语言描述的数据时，通常使用双引号（`""`）包围，表示这些数据和关键词、程序定义的函数、算法等的名称不同。这类数据称作“字符串”，本质上是字符构成的序列。双引号包围的字符串、直接写到程序中的数，术语称为 `字面值`。
