...
- 为区别代数式中的不变量和变量，使用 `$ x $` 和 `$ y $` 分别表示变量和和结果：
...
- 常数函数： `$ \large{ y = c } $`
- 一次函数： `$ \large{ y = ax + c \quad (a \ne 0)} $`
- 二次函数： `$ \large{ y = ax^2 + bx + c  \quad (a \ne 0)} $`
...
- 字符：包含 ASCII 字符在内的人类使用的所有文字的字母、象形字、符号等；由 Unicode 标准组定义，超过 14 万个字符。
- 字符串：一组字符构成的序列。
