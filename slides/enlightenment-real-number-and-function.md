...
- 倒数：对任意实数 `$ a \ne 0 $`，一定有 `$ x $` 使得 `$ ax = 1 $`；这时，称 `$ x $` 为 `$ a $` 的倒数。实际上有 `x = 1 \div a = \frac{1}{a} $`，而 `$ a $` 是 `$ x $` 的倒数。
...
2) 将正整数分解为指定进制下数码序列的功能写成一个函数，并编写算法调用相应的函数。
