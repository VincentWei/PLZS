...
`$$
\begin{align}
    (72.45)_8 &= (\qquad)_{10} \qquad\qquad (1A6.3B8)_{16} &&= (\qquad)_{10}        \\
    15.68 &= (\qquad)_2 \qquad\qquad                 10.68 &&= (\qquad)_8
\end{align}
$$`

- 3) 编写考鼎码算法，将一个十进制小数转换为给定的进制 `b` 表达，精确到小数点 `n` 位。

- 4) 编写考鼎码算法，将一个无限循环小数转为分数。程序应分别输入整数部分、小数中的不循环部分及循环部分，如 `$ 100.356\dot{1}\dot{7} $` 分为三部分输入：`$ 100 $`、`$ 356 $` 和 `$ 17 $`。
