...
- 3) 假设考鼎码提供 `常用对数` 关键词，可用来计算给定实数的常用对数。请编写算法，利用换底公式计算给定真数（`N`）对给定底数（`a`）的对数。
</div>
...
- 字节：8位二进制数。
- ASCII：美国制定的最早的字符编码：包括拉丁字母、阿拉伯数字、常用标点符号等，一共 128 个字符，用 `0` 到 `127` 的数值表示；一个 ASCII 字符可用一个字节表示。
- 字符：包含 ASCII 字符在内的人类使用的所有文字字母、符号等（由 Unicode 标准定义）。
- 字符串：一组字符构成的序列。
- 在可执行考鼎码定义字节、字符或者字符串：
