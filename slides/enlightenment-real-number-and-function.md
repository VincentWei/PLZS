...
- 程序中需要用到人名、地名等使用自然语言描述的数据时，通常使用双引号（`""`）包围，表示这些数据和关键词、程序定义的函数、算法等的名称不同。这类数据称作“字符串”，本质上是字符构成的序列。
