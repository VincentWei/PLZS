...
- 3) 在数学上，函数 `$ f(x) = e^x $` 等价于 `$ f(x) = \frac{x^0}{0!} + \frac{x^1}{1!} + \frac{x^2}{2!} + \cdots $`；也就是说，我们可以用和式 `$ \sum^{n}_{k=0}\frac{x^k}{k!} $` 来求得 `$ e^x $` 的近似值，而且当 `$ n $` 越大时，得到的结果越精确。我们将该代数和式称为 `$ e^x $` 的逼近式。请据此编写考鼎码算法，该算法可针对给定的 `$ x $` 利用逼近式近似计算 `$ e^x $` 的结果。逼近式中的 `$ n $` 可通过算法的第二个参数传入。
...
- 3) 在数学上，函数 `$ f(x) = e^x $` 等价于 `$ f(x) = \frac{x^0}{0!} + \frac{x^1}{1!} + \frac{x^2}{2!} + \cdots $`；也就是说，我们可以用和式 `$ \sum^{n}_{k=0}\frac{x^k}{k!} $` 来求得 `$ e^x $` 的近似值，而且当 `$ n $` 越大时，得到的结果越精确。我们将该代数和式称为 `$ e^x $` 的逼近式。请据此编写函数 `自然常数的任意次幂`，该函数可针对给定的浮点数 `$ x $` 利用逼近式近似计算 `$ e^x $` 的结果，逼近式中的 `$ n $` 可通过函数的第二个参数传入。然后，编写主算法，针对用户指定的 `$ x $` 分别用 `$ 5 $`、`$ 10 $`、`$ 20 $` 作为逼近式中的 `$ n $` 调用该函数，并和 `math.power()` 函数给出的结果做对比，展示误差（两者结果之差的绝对值，用 `errors` 表示）。输出示例如下：

```
n = 5  时，f(1) 的估计值为：；误差为：
n = 10 时，f(1) 的估计值为：；误差为：
n = 20 时，f(1) 的估计值为：；误差为：
```
