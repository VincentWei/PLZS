...
1. 在全体自然数中，加法和乘法是封闭的。<!-- .element: class="fragment fade-in" -->
1. 负数的定义和写法。<!-- .element: class="fragment fade-in" -->
1. 相反数和绝对值的概念。<!-- .element: class="fragment fade-in" -->
1. 通过数轴理解加法和减法的意义。<!-- .element: class="fragment fade-in" -->
1. 在全体整数中，加法、减法和乘法运算是封闭的。<!-- .element: class="fragment fade-in" -->
...
2) 写出 `30` 以内的所有质数。
3) 挑选出 `1`、`2`、`7`、`8`、`31`、`87`、`91`、`100`、`201`、`992` 当中的质数。
4) 编写考鼎码算法，计算从 `1` 开始的 `n` 个奇数之和。
5) 编写考鼎码算法，判定一个给定的整数是否为一个完全立方数。
...
1. 编写考鼎码算法，找出小于等于给定正整数 `n` 的所有自恋数。
...
1. 使用本讲给出的欧几里得算法，当给定的初始值 `a, b` 取下列各值时，写出算法每一个步骤各变量的值以及执行结果（可使用表格）。  
      a) `a=22, b=11`<br/>
      b) `a=125, b=20`
1. 为防止在取模过程中出现除数为零的错误，需要调整本讲给出的欧几里得算法，请尝试添加代码使之可以避免出现除数为零的情况。

		
## 第九讲

- 标题：中国剩余定理
- 旁白：清华大大，听说咱有个孙子定理，老外也经常用。您快给讲讲。
...
- 有物不知其数，三三数之剩二，五五数之剩三，七七数之剩二。问物几何？<br/>-- 《孙子算经》
- 翻译后：求满足以下条件的整数：除以 `3` 余 `2`，除以 `5` 余 `3`，除以 `7` 余 `2`。
...
### 孙子定理朴素解法<!-- .element: class="fragment fade-out" -->
...
### 孙子定理朴素解法的考鼎码<!-- .element: class="fragment fade-out" -->
