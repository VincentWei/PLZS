...
- 3) 增强加法实现的乘法考鼎伪代码算法，使之支持负数作为被乘数或乘数。
...
- 2) 写出 `30` 以内的所有质数。
- 3) 编写考鼎伪代码算法，计算从 `1` 开始的 `n` 个奇数之和。
- 4) 编写考鼎伪代码算法，判定一个给定的整数是否为一个完全立方数。
...
- 2) 编写考鼎伪代码算法，判定给定的正整数是不是完美数。
- 2) 如果某个正整数 `$ a $` 的真因子之和为 `$ b $`，而 `$ b $` 的真因子之和为 `$ a $`，则称 `$ a, b $` 为一对亲和数。古希腊的毕达哥拉斯于两千年前发现了第一对亲和数：`$ 220, 284 $`。请编写考鼎伪代码算法，判定给定的两个正整数是不是亲和数。
...
- 4) 编写考鼎伪代码算法，找出小于等于给定正整数 `n` 的所有自恋数。
...
1. 优化本节给出的质数判定算法。
1. 编写考鼎伪代码算法，分解大于 `2` 的任意偶数为两个质数之和。
...
- 2) 设定一个已知质数构成的序列，初始时，该序列中只有 `2` 一个元素。然后从 `3` 开始依次检测每个整数，用已知质数序列中的每个质数去除这个整数。如果某个已知质数是这个整数的因子，则该整数为一个合数，否则该整数为下一个新发现的质数，并将其加入到已知质数的序列当中。如此，我们可以生成一个包含从小到大排列的所有质数的序列。请按此方法编写考鼎伪代码算法，生成小于等于给定正整数 `n` 的所有质数序列。
- 3) 请思考上述算法的至少一个优化点。
...
1. 写出质因子分解算法朴素版的考鼎伪代码；该考鼎码分解一个大于 `1` 的正整数 `n`，返回一个序列，其中包含该正整数的所有的质因子。注意：如果是该正整数是一个质数，则结果序列中应仅包含自己。
1. 写出质因子分解算法优化版的考鼎伪代码；要求同上。
1. 证明：若一个正整数 `$ a $` 的平方为偶数，则 `$ a $` 一定是偶数。
...
1. 满足中国剩余定理所述问题的整数是唯一的吗？如果不唯一且得到一个解，那么满足该定理的其他整数如何求得？请给出其考鼎伪代码。
1. 思考如果模数不是固定的 `3`、`5`、`7`，对应的余数也不是固定的 `2`、`3`、`3`，甚至可能给出更多的模数和对应的余数，我们需要如何改造考鼎伪代码算法以适应这种情况？
...
1. 已知填充幻方的连续整数的起始值 `$ n $`，写出计算 `$ 3 \times 3 $` 幻方各行、列或对角线之和的考鼎伪代码算法。
1. 在以上条件下，写出确定对应幻方中心位置数值的考鼎伪代码算法。
