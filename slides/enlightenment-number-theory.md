...
- 4) 数学上余数的定义：`$ 若 a \bmod b = c，则有 c = a - \lfloor \frac{a}{b} \rfloor $`， 其中 `$ \lfloor \rfloor $` 为 `向下取整` 运算符，如 `$ \lfloor 3.476 \rfloor = 3,\ \lfloor -3.14159 \rfloor = -4 $`。请据此调整第二单元给出的除法和取模算法，使之支持负数。
