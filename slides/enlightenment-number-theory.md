...
- 2) 如果某个正整数 `$ a $` 的真因子之和为 `$ b $`，而 `$ b $` 的真因子之和为 `$ a $`，则称 `$ a, b $` 为一对亲和数。古希腊的毕达哥拉斯于两千年前发现了第一对亲和数：`$ 220, 284 $`。请编写考鼎码算法，判定给定的两个正整数是不是亲和数。
...
1. 写出质因子分解算法朴素版的考鼎码；该考鼎码分解一个大于 `$ 1 $` 的正整数 `$ n $`，返回一个序列，其中包含该正整数的所有的质因子（每个质因子出现一次）。注意：如果是该正整数是一个质数，则结果序列中应仅包含自己。
